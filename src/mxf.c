...
int write_mxf(char *input_path, char *output_file, int frame_rate) {

    const ui32_t FRAME_BUFFER_SIZE = 40 * Kumu::Megabyte;
    const ASDCP::Dictionary *g_dict = 0;

    ASDCP::Rational picture_rate = Rational(24000,1001);
    ...
    //set parameterized frame_rate
    if (frame_rate == 24) {
        picture_rate = Rational(24,1);
    }
...
#ifdef ATOM_PROFILE
            result = Writer.OpenWriteCustom(output_file, info, PDesc, 16384, tmp_dscr);
#else
	        result = Writer.OpenWrite(output_file, info, essence_descriptor, essence_sub_descriptors, picture_rate, 16384, AS_02::IS_FOLLOW, 10);
#endif
        }
